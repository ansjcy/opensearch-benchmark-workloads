...
* `bulk_total_steps` (default: 1): the number of steps to increase the number of clients.
* `num_clients_per_step` (default: 5): tthe number of clients to increase in each steps of `bulk_total_steps`. For example: `--workload-params="num_clients_per_step:5,bulk_total_steps:2"` means to run ingestion in 2 steps, in each step increase 5 clients. (i.e. in step 1, use 5 clients; in step 2, use 10 clients).
